...
def validate_scene(scene):
    ...
    clean = {
        "version": 1,
        "project": str(scene.get("project", "Nova Frontier"))[:100],
        "title": str(scene.get("title", "Untitled concept"))[:120],
        "objects": [],
    }
    for index, obj in enumerate(objects):
        if not isinstance(obj, dict) or obj.get("kind") not in ("box", "cylinder", "pyramid"):
            raise ValueError("Supported shapes: box, cylinder, pyramid")
        color = obj.get("color", "#67cbd4")
        if not isinstance(color, str) or not re.fullmatch(r"#[0-9a-fA-F]{6}", color):
            raise ValueError("Colors must be six-digit hex values")
        clean["objects"].append({
            "name": str(obj.get("name", f"Part {index + 1}"))[:80],
            "kind": obj["kind"],
            "position": _vector(obj.get("position"), [0, 0, 0]),
            "size": _vector(obj.get("size"), [1, 1, 1], True),
            "color": color,
        })
    return clean
...
def _mesh(obj):
    ...
    return [tuple(v[i] * obj["size"][i] + obj["position"][i] for i in range(3)) for v in vertices], faces
...
def preset(name="outpost"):
    ...
    if name == "outpost":
        ...
        raise ValueError("Choose outpost, spaceship, or terrain")
    return validate_scene({"title": f"Nova Frontier - {name.title()}", "objects": objects})


def obj_text(scene):
    scene = validate_scene(scene)
    lines = ["# Nova Frontier concept mesh - Y up; units: meters"]
    offset = 1
    for i, obj in enumerate(scene["objects"]):
        vertices, faces = _mesh(obj)
